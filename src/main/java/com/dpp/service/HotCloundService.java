...
*/
@Service
public class HotCloundService extends AbstractChannelService {
	
	@Autowired
	private HttpClientService httpClientService;
	@Value("${hotClound.queryMatchResult.url}")
	private String queryUrl;
	private static final Logger logger = LoggerFactory.getLogger(HotCloundService.class);
	
	
	public MatchResultReqDomain parseContent(String fileName,String line,String mediaCode,String today) {
		...
		try {
			...
			req.setCamp_ids(camp_ids);
			req.setDeviceid_type(Integer.valueOf(s[1]));
			req.setDevice_id(s[2]);
			req.setMedia_id(mediaCode);
			return req;
		} catch(Exception e) {
			logger.error("parseContent fileName={}, line:{}",fileName,line);
			return null;
	...
	public String executeMatchPreMachine(String fileName,String line,String mediaCode,String today) {
		MatchResultReqDomain  req = parseContent(fileName, line,mediaCode,today);
		if(null != req) {
